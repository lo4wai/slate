...
void ConfirmActionPage::OnEnter(SceneContext& ctx) {
    ...
    CreateBottomHint("按確認 返回   長按確認 執行");
...
void VolumePage::OnEnter(SceneContext& ctx) {
    if (!EnterSettingsScaffold(ctx, "音量調節"))
        return;
    ...
    hint_label_ = CreateBottomHint("上/下 調節   按確認 返回   長按確認 試聽");
...
bool DeviceInfoPage::Refresh(SceneContext& ctx) {
    ...
    std::snprintf(buf, sizeof(buf),
                  ...
                  "狀態    %s\n"
                  "\n"
                  "DRAM    總量 %u KB，可用 %u KB（%u%%）\n"
                  "PSRAM   總量 %u KB，可用 %u KB（%u%%）\n"
                  "存儲    總量 %u KB，可用 %u KB（%u%%）\n"
                  "\n"
                  "固件    %s\n"
                  "MAC     %s\n"
                  "服務器  %s",
                  wifi_on ? "已連接" : "未連接", rssi, wifi_ssid_.empty() ? "-" : wifi_ssid_.c_str(),
...
RestartDevicePage::RestartDevicePage()
    : ConfirmActionPage("restart_device", "重啓設備",
                        "確認要重啓設備嗎？\n\n"
                        "Wi-Fi 配置和已下載\n"
                        "的內容緩存都保留\n"
                        "重啓完成後自動恢復") {
...
FactoryResetPage::FactoryResetPage()
    : ConfirmActionPage("factory_reset", "恢復出廠",
                        "確認要恢復出廠嗎？\n\n"
                        "Wi-Fi 配置、設備綁定\n"
                        "小智配置及內容緩存\n"
                        "將全部清除\n"
